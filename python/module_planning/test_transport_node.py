...
class TransportNodeTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_failed_outgoing_pipe_does_not_leave_transport_busy(self):
        node = self.transport.TransportNode(
            is_master=True, debug=False, core=FakeCore()
        )
        start = node._out_pipe_start(0)
        node._outgoing_pipes[start + self.transport._PIPE_ACTIVE] = 1
        node._outgoing_pipes[start + self.transport._PIPE_STREAM_ID] = 17
        node._outgoing_pipes[start + self.transport._PIPE_STATE] = \
            self.transport._PIPE_FAILED
        node._pipe_failures[0] = 6

        self.assertFalse(node._transport_busy())
        with self.assertRaisesRegex(RuntimeError, "pipe failed 6"):
            await node.send_pipe(17, b"data")
        self.assertEqual(
            node._outgoing_pipes[start + self.transport._PIPE_ACTIVE], 0
        )
